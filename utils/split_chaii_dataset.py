...
with open(os.path.join(data_dir, 'test_ids.txt'), 'w') as file:
    file.writelines([f'{test_id}\n' for test_id in test_ids.values.tolist()])
...
train_translated_df[~train_mask].to_csv(os.path.join(data_dir, 'train_translated_test.csv'), index=False)
...
train_transliterated_df[~train_mask].to_csv(os.path.join(data_dir, 'train_transliterated_test.csv'), index=False)